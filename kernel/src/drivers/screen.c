...
void clear_line(int line) {
    ...
    for (int i = 0; i < 80; i++) {
        video_memory[(line * 80 + i) * 2] = ' ';
        video_memory[(line * 80 + i) * 2 + 1] = 0xFF; // White text
    }
}
...
void boot_print_clear() {
    ...
    for (int i = 10; i < 25; i++) {
        for (int j = 0; j < 80; j++) {
            ...
            video_memory[(i * 80 + j) * 2 + 1] = 0xFF; // White text
        }
    }
    curr_line = 10;
}
...
void boot_println(char* str){
    ...
    while (*str != '\0') {
        video_memory[(line * 80 + i) * 2] = *str;
        video_memory[(line * 80 + i) * 2 + 1] = 0xF0; // White text
        str++;
        i++;
    }
    if (curr_line >= 25) {
...
void showboot() {
    ...
    for (int y = 0; y < 6; y++) {
        for (int x = 0; x < 49; x++) {
            ...
            video_memory[((start_y + y) * 80 + (start_x + x)) * 2 + 1] = 0xF0; // White text on black background
        }
    }
